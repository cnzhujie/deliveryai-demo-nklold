...
export function TopBar({ table, view, serviceCount, language, elderly, darkMode, onToggleLanguage, onToggleElderly, onToggleDarkMode, onView, onService, onConsole }: TopBarProps) {
  ...
  return (
    <>
      <div className="bg-charcoal-900 px-4 py-2 text-center text-xs font-semibold tracking-wide text-rice-100 dark:bg-charcoal-900 dark:text-rice-100">
        {t('common.banner')}
      </div>
      <header className="sticky top-0 z-30 border-b border-charcoal-900/5 bg-rice-50/95 backdrop-blur-xl dark:border-rice-200/10 dark:bg-charcoal-700/95">
        <div className="mx-auto flex h-16 max-w-7xl items-center gap-3 px-4 lg:px-6">
          <button onClick={() => onView('menu')} className="flex items-center gap-2 text-left">
            ...
            <span className="hidden sm:block"><strong className="block leading-4 text-charcoal-900 dark:text-rice-100">{t('common.brand_name')}</strong><small className="text-charcoal-500 dark:text-rice-200">{t('common.subtitle')}</small></span>
          </button>
          <span className="ml-1 flex items-center gap-1 rounded-full bg-rice-200 px-3 py-2 text-xs font-bold text-charcoal-700 dark:bg-charcoal-900 dark:text-rice-200"><MapPin size={13} className="text-chili-500" />{tableLabel}</span>
          ...
          <Button variant="outline" size="icon" onClick={onService} className="relative" aria-label={t('common.aria_service')}>
            <PhoneCall size={18} />{serviceCount > 0 && <span className="absolute -right-1 -top-1 h-3 w-3 rounded-full border-2 border-white bg-chili-500 dark:border-charcoal-700" />}
          </Button>
          <Dialog>
            ...
            <DialogContent title={t('common.member_title')}>
              ...
              <div className="mt-4 grid grid-cols-2 gap-3">
                <div className="rounded-2xl bg-white p-4 shadow-sm dark:bg-charcoal-900 dark:shadow-none"><p className="text-xs text-charcoal-500 dark:text-rice-200">{t('common.queue')}</p><p className="mt-2 text-2xl font-extrabold text-charcoal-900 dark:text-rice-100">A018</p><p className="text-xs text-chili-500">{t('common.queue_ahead')}</p></div>
                <div className="rounded-2xl bg-white p-4 shadow-sm dark:bg-charcoal-900 dark:shadow-none"><p className="text-xs text-charcoal-500 dark:text-rice-200">{t('common.benefits')}</p><p className="mt-2 text-2xl font-extrabold text-charcoal-900 dark:text-rice-100">4 <small className="text-sm">{t('common.tickets')}</small></p><p className="text-xs text-amber-500">{t('common.coupon')}</p></div>
              </div>
            </DialogContent>
          </Dialog>
